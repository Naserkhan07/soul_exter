...
async def test_instagram_dm_is_persisted_and_skipped_on_resume(tmp_path: Path) -> None:
    settings = settings_for(tmp_path)
    repository = JobRepository(settings.database_path)
    job = repository.create(0, 0, "https://youtu.be/example")
    ...
    services = WorkflowServices(
        ...
        youtube_uploader=FakeYouTubeUploader(),  # type: ignore[arg-type]
        instagram_uploader=FakeInstagramUploader(),  # type: ignore[arg-type]
        instagram_messenger=messenger,  # type: ignore[arg-type]
    )
    pipeline = WorkflowPipeline(settings, repository, services)

    first_result = await pipeline.process(job.id)
    second_result = await pipeline.process(job.id, reuse_downloaded=True)
    clip = repository.list_clips(job.id)[0]

    assert first_result.status == JobStatus.COMPLETE
    assert second_result.status == JobStatus.COMPLETE
    assert clip.instagram_dm_message_id == "dm-message-id"
    assert messenger.sent == [f"soul_exter/dm/{job.id}/clip-001"]
    assert messenger.waits == 1
