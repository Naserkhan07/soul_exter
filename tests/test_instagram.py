...
async def test_instagram_resumable_reel_publish_flow(tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/v25.0/1789/media":
            assert b"share_to_feed=true" in request.content
            return httpx.Response(
                200,
                json={
                    "id": "container-id",
                    "uri": ("https://rupload.facebook.com/ig-api-upload/v25.0/container-id"),
                },
            )
        ...
        if request.url.path == "/v25.0/container-id":
            return httpx.Response(200, json={"status_code": "FINISHED"})
        if request.url.path == "/v25.0/1789/media_publish":
            return httpx.Response(200, json={"id": "media-id"})
        if request.url.path == "/v25.0/media-id":
            return httpx.Response(
                200,
    ...
    assert calls == [
        ("POST", "/v25.0/1789/media"),
        ("POST", "/ig-api-upload/v25.0/container-id"),
        ("GET", "/v25.0/container-id"),
        ("POST", "/v25.0/1789/media_publish"),
        ("GET", "/v25.0/media-id"),
    ]
